...
public class GameTapSwipe : MonoBehaviour
{
        ...
        if (play && isPlaying)
        {
            ...
            if (count < note.note.time.Length)
            {
                ...
                if (aud.time >= instTime)
                {
                    ...
                    {
                        if (note.note.instname[i] != instCount || (note.note.time[i] - instTime) > 2.0f)
                        {
                            Debug.Log("Its same");
                            break;
                        }
                    ...
                    ob.GetComponent<TapSwipeInstrument>().AddNote(tmpCount);
                }
            }
